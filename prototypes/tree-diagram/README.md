...
This is a mod written in `TypeScript`, rendered with `d3` and bundled with `Webpack`.
...
## How to enterpreate the example data (data_tree.csv)
- Each node consists of a child and a parent.
- if the child column is empty that node does not have children
- if the parent column is empty that node does not have any parents (root node)
- The ordering of the nodes are important see example picture (ex_tree.png) and compare.